...
def parse_peers(peers_str):
    """Parse a comma-separated 'node_id=host:port' string into a dict."""
    peers = {}
    if not peers_str:
        return peers
    for peer in peers_str.split(","):
        node_id, _, address = peer.partition("=")
        if node_id.strip() and address.strip():
...
def serve():
    ...
    parser.add_argument(
        "--peers",
        default="",
        help="Comma-separated peers, e.g. node-2=localhost:50052,node-3=localhost:50053",
    )
    args = parser.parse_args()

    peers = parse_peers(args.peers)
    logger.info("Starting server on %s:%d as '%s'", args.host, args.port, args.node_id)
    if peers:
        logger.info("Configured peers: %s", peers)
    else:
        logger.info("Running in standalone mode (no peers configured).")

    # The RaftNode owns the replicated state machine and serves the Raft RPCs.
    ...
    server.start()
    logger.info("Server is running and listening on port %d", args.port)
